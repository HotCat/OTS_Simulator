...
func _initialize() -> void:
	...
	if arguments.size() < 2:
		_fail("Usage: Godot --headless --path . --script res://tools/bake_motion_cache.gd -- <cache.json> <library.tres>")
		return
	...
	var animation_name := StringName(str(clip.get("animation_name", "female_walk_linear")))
		...
		for frame_index in frames.size():
			...
			if values is Array and values.size() >= 4:
				animation.rotation_track_insert_key(
					track,
					frame_index / fps,
					Quaternion(float(values[0]), float(values[1]), float(values[2]), float(values[3])).normalized()
				)
		if is_cycle:
...
func _trajectory_origin(path_string: String) -> Vector3:
	if path_string.is_empty():
		return Vector3.ZERO
	var scene := load("res://demos/my_manual_rig_pose.tscn") as PackedScene
	if scene == null:
		return Vector3.ZERO
